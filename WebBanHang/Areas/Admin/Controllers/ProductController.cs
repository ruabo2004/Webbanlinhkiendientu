...
namespace WebBanLinhKienDienTu.Areas.Admin.Controllers
{
    ...
    public class ProductController : AdminBaseController
    {
        ...
        public ActionResult Edit(int? id)
        {
            ...
            // Load IsNew from database using raw SQL
            try
            {
                var db = new ecommerceEntities();
                var isNewSql = "SELECT IsNew FROM Products WHERE ProductID = @p0";
                var isNewResult = db.Database.SqlQuery<bool?>(isNewSql, id).FirstOrDefault();
                model.IsNew = isNewResult ?? false; // Default to false if column doesn't exist or is null
            }
            catch
            {
                // If column doesn't exist yet, default to false
                model.IsNew = false;
            }
        ...
        public ActionResult Edit(AdminProductViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                // Update IsNew field using raw SQL (since it's not in EDMX)
                var db = new ecommerceEntities();
                var updateSql = "UPDATE Products SET IsNew = @p0 WHERE ProductID = @p1";
                db.Database.ExecuteSqlCommand(updateSql, model.IsNew, model.ProductID);
        ...
        public ActionResult GetListAttr(int? id, int product_id, bool available = false)
        {
            ...
            var listAttr = Repository.Create<WebBanLinhKienDienTu.Models.Attribute>().FetchAll().Where(a => a.AttriGroupID == id || a.AttriGroupID == null);
            ...
            List<WebBanLinhKienDienTu.Models.Attribute> availableAttr = listAttr.ToList();
            if (available)
            {
        ...
        public ActionResult GetListColor(int id, bool available = false)
        {
            ...
            if (product == null)
            {
                return Content(String.Format("<option value=\"\">Sản phẩm không tồn tại</option>"));
            }
        ...
        public ActionResult InsertAttr(ProductAttribute pAttr)
        {
            ...
            var attr = Repository.Create<WebBanLinhKienDienTu.Models.Attribute>().FindById(pAttr.AttrID);
            if (product == null)
            {
                result.status = false;
            ...
            {
                result.status = false;
                result.message = "Không tồn tại thuộc tính";
                return Content(JsonConvert.SerializeObject(result), "application/json");
            }
        ...
        public ActionResult InsertColor(ProductColor pColorBind)
        {
            ...
            {
                result.status = false;
                result.message = "Màu này đã được thêm trước đó";
                return Content(JsonConvert.SerializeObject(result), "application/json");
            }
        ...
        public ActionResult DeleteColor(ProductColor pColorBind)
        {
            ...
            {
                result.status = false;
                result.message = "Lỗi không thể xóa";
                return Content(JsonConvert.SerializeObject(result), "application/json");
            }
            result.status = true;
